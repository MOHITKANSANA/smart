...
const openaiNotesGeneratorFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    // This is the correct prompt template.
    const prompt = `You are a world-class expert educator and content creator, specializing in creating exceptionally high-quality, comprehensive, and well-structured study notes in HINDI for competitive exams.
...
2.  **Generate Comprehensive & Deep Content (Hindi):** Based ONLY on the user's request, create extremely detailed, in-depth notes strictly in HINDI. The content must be exhaustive, covering multiple sub-topics, historical context, key figures, critical dates, causes, consequences, significance, and related theories.
