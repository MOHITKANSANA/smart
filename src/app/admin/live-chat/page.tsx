...
function ChatWindow({ activeSession }: { activeSession: ChatSession }) {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
             // We only update the session, not create it from the admin side.
            await Promise.all([
                addDoc(collection(firestore, `live-chats/${activeSession.id}/messages`), messageData),
                updateDoc(sessionRef, { 
                    lastMessage: `आप: ${newMessage}`,
                    lastMessageAt: serverTimestamp(),
                    isReadByAdmin: true // Admin just sent it, so it's "read"
                 })
            ]);
            setNewMessage('');
        } catch (error) {
...
export default function AdminLiveChatPage() {
    ...
    // Set first session as active by default on desktop
    useEffect(() => {
    ...
    const handleSessionClick = (session: ChatSession) => {
        ...
        if (window.innerWidth < 768) { // On mobile, show only the chat window
             // The layout will handle showing/hiding panes based on activeSession
        }
    }
    
    const isChatWindowVisible = activeSession && window.innerWidth < 768;


    return (
        <AppLayout>
            <main className="flex-1 flex h-[calc(100vh-var(--top-bar-height,4rem))]">
                {/* Left Pane: Chat Sessions List */}
                <div className={cn(
                    "w-full md:w-1/3 lg:w-1/4 border-r flex flex-col transition-all duration-300",
                    isChatWindowVisible && "hidden"
                )}>
                    ...
                    isChatWindowVisible && "w-full"
                     ...
                     <div className="p-4 border-b flex items-center gap-2 md:hidden sticky top-0 bg-background/80 backdrop-blur-sm">
                        <Button variant="ghost" size="icon" onClick={() => setActiveSession(null)}>
                         ...
                         <h1 className="font-bold text-lg">चैट</h1>
                     </div>
